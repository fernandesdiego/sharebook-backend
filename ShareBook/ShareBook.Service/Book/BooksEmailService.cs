...
namespace ShareBook.Service
{
    public class BooksEmailService : IBooksEmailService
    {
        ...
        private const string NewBookInsertedTitle = "Novo livro incluído!";
        private const string WaitingApprovalTemplate = "WaitingApprovalTemplate";
        private const string WaitingApprovalTitle = "Aguarde aprovação do livro.";
        ...
        {
            // TODO: reformular o mailling new book
        ...
        private async Task SendEmailNewBookInsertedToAdministrators(Book book, UserStatsDTO userStats)
        {
            var model = new
            {
                Book = book,
                UserStats = userStats,
                UserCreationDateFormated = string.Format("{0:dd/MM/yyyy}", userStats.CreationDate)
            };
